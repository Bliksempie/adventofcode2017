<?php
    ...
    $total_check = 0;
    foreach ($biggest as $k => $v) {

        // We assume that we receive valid data, so index of $smallest will match index of $biggest.
        $check[$k] = $biggest[$k] - $smallest[$k];
        $total_check += $check[$k];
    }
    ...
    echo '
        <p>
            <div style="padding: 20px; background-color: #eee; height: auto; width: 90%; overflow: auto">
                ...
                . $link . '
                <h2>Solve:</h2>
                The spreadsheet consists of rows of apparently-random numbers. To make sure the recovery process is on the right track, they need you to calculate the spreadsheet\'s checksum. For each row, determine the difference between the largest value and the smallest value; the checksum is the sum of all of these differences. What is the checksum for the spreadsheet in your puzzle input?
            </div>
        </p>
        <p>
            <div style="padding: 20px; background-color: #eee; height: auto; width: 90%; overflow: auto">

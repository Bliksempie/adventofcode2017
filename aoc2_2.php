<?php
    ...
    // Clean up values for each rows' columns - trim spaces tabs etc.
    foreach ($cols as $k => $v) {
        foreach ($v as $kk => $vv) {
            $cols[$k][$kk] = trim($vv);
        }
    ...
    // Determine those divisible wholly.
    $row_matches = array();
    foreach ($cols as $k => $v) {
        for ($i = 0; $i < (count($v) - 1); $i++) {
            for ($j = 0; $j < (count($v) - 1); $j++) {
                if ($cols[$k][$i] != $cols[$k][$j]) {
                    if ($cols[$k][$i] >= $cols[$k][$j]) {
                        $match = $cols[$k][$i] % $cols[$k][$j];
                        if ($match == 0) {
                            $row_matches[$k][0] = $cols[$k][$i];
                            $row_matches[$k][1] = $cols[$k][$j];
                        }
                    } else {
                        $match = $cols[$k][$j] % $cols[$k][$i];
                        if ($match == 0) {
                            $row_matches[$k][0] = $cols[$k][$j];
                            $row_matches[$k][1] = $cols[$k][$i];
                        }
                    }
                }
            }
    ...
    echo '
        <p>
            <div style="padding: 20px; background-color: #eee; height: auto; width: 90%; overflow: auto">
                ...
                . $link . '
                <h2>Solve:</h2>
                It sounds like the goal is to find the only two numbers in each row where one evenly divides the other - that is, where the result of the division operation is a whole number. They would like you to find those numbers on each line, divide them, and add up each line\'s result. What is the sum of each row\'s result in your puzzle input?
            </div>
        </p>
        <p>
            <div style="padding: 20px; background-color: #eee; height: auto; width: 90%; overflow: auto">
